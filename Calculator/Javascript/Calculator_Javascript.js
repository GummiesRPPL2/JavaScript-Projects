...
function Input_Digit(digit) {
    ...
    //to the key that was pressed on.
...
function Handle_Operator(Next_Operator) {
    ...
    //then updates the operator and exits from the funcion.
...
function Update_Display() {
    //Makes use of the calculator-screen clas to target the
...
Update_Display();
//This secontion monitors button clicks
const keys = document.querySelector('.calculator-keys');
keys.addEventListener('click', (event) => {
    //The target variable is an objext that represents the element
    //that was clicked.
    const {target} = event;
    //if the eloement that was clicked on is not a button, exit the function.
    ...
    //Ensures that AC clears all unputs from the calculator screen.
